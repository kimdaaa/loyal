...
class Content():
    ...
    def get_latest_season_id(self, content):
        seasons = content.get("Seasons", []) if isinstance(content, dict) else []

        # Ranked MMR is keyed by act ID. The content feed can also mark an
        # episode active, so prefer the active act to avoid empty ranked data.
        for season in seasons:
            if season.get("IsActive") and season.get("Type") != "episode":
                self.log(f"retrieved active act season id: {season.get('ID')}")
                return season.get("ID")

        for season in seasons:
            if season.get("IsActive"):
                self.log(f"retrieved fallback active season id: {season.get('ID')}")
                return season.get("ID")
    ...
    def get_act_episode_from_act_id(self, act_id):
        ...
        if not act_id or not isinstance(self.content, dict):
            return final
        ...
        for season in self.content["Seasons"]:
            ...
            if act_found and season.get("Type") == "episode":
                episode_num = parse_season_number(season.get("Name"))
